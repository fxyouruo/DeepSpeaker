...
from utils.utils import list_files, audio_predicate, remove_silence, get_mfcc, threadsafe_generator, get_energy


class TimitBatchGenerator:
    np.random.seed(0)

    @staticmethod
    def feature_extractor(y, sr):
        # mfccs = get_mfcc(y, sr, delta=True, delta_delta=True)
        mfccs = get_mfcc(y, sr, delta=False, delta_delta=False)
        energies = get_energy(y, sr, delta=True, delta_delta=True)
        return np.hstack((mfccs, energies))

    def __init__(self, dir_path, num_speakers=100, frames=32, silence=True, batch_size=32):
        self.frames = frames
        ...
        for i, f in self.speaker_files.items():
            ...
            new_shape = tuple(list(speaker_feature.shape) + [-1])
            speaker_features[i] = (speaker_feature.reshape(new_shape), self.speakers[speaker])
    ...
    def generator(self):
        ...
        x = np.zeros(shape=(self.batch_size, self.frames, self.dim, 1))
